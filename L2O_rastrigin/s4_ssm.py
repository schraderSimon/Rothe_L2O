...
# S4-inspired L2O Optimizer
class L2OOptimizer(nn.Module):
    def __init__(self, state_size, linear_size=10, rank=2):
        ...
        # S4-inspired parameters
        self.A_diag = nn.Parameter(torch.randn(state_size) * 0.01)  # Diagonal A matrix
        self.B = nn.Parameter(torch.randn(linear_size, rank) * 0.01)  # Low-rank B
        self.V = nn.Parameter(torch.randn(rank, state_size) * 0.01)  # Low-rank B projection
        self.C = nn.Parameter(torch.randn(state_size, 1) * 0.01)  # Output projection
    ...
    def forward(self, x, grad, hidden):
        ...
        # S4-inspired update: h_t = diag(A) * h_{t-1} + B * V * u_t
        h_new = h_prev * torch.exp(self.A_diag)  # Diagonal A update
        input_term = inp_flat @ self.B @ self.V  # Low-rank B * V
        h_new = h_new + input_term
...
def train_l2o_tune(config):
    ...
    l2o_net = L2OOptimizer(state_size, linear_size=linear_size, rank=rank)
    l2o_net.to(device)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "state_size": tune.choice([64, 128, 256]),
        "rank": tune.choice([1, 2, 4]),
        "T": T,
