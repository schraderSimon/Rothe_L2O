...
def main_experiment_l2o(num_problems=1000, dims=[1,20], T=100, T_training=10, wM=1.0, type="training"):
    ...
    if type == "training":
        df.to_csv("S4_L2O_training_T=%d_wM=%.2f.csv" % (T_training, wM), index=False)
    elif type == "testing":
        df.to_csv("S4_L2O_testing_T=%d_wM=%.2f.csv" % (T_training, wM), index=False)
    print(f"Saved CSV: S4_L2O_{type}_T=%d_wM=%.2f.csv" % (T_training, wM))
