...
parser = argparse.ArgumentParser(description="Plot S4 optimizer results with optional filtering by wf or T.")
...
def plot_optimizer_data(ax, method, wf_filter, T_filter):
    ...
    pattern = f"S4_L2O_{method}_T={T_filter}_wM={wf_filter}.csv"
    for file in sorted(glob.glob(pattern)):
        ...
        label = f"S4-SSM L2O T={T_str} k={wf_str}"
...
# Adjust the subplots manually to make room for the legend.
fig.subplots_adjust(top=0.85, bottom=0.08, left=0.10, right=0.95)

# Save the figure and include the legend in the bounding box.
plt.savefig("plots/s4_combined_results_T=%s_wM=%s.png" % (T_filter, wf_filter),
            dpi=300, bbox_inches='tight', bbox_extra_artists=[leg])
plt.savefig("plots/s4_combined_results_T=%s_wM=%s.pdf" % (T_filter, wf_filter),
            dpi=300, bbox_inches='tight', bbox_extra_artists=[leg])
plt.show()

print("Combined plot saved as s4_combined_results.png")