...
# -----------------------------------------------------------------------------
try:
    type=sys.argv[1]  # "narrow" or "wide"
except IndexError:
    type="wide"  # "narrow" or "wide"
if type == "narrow":
    ...
    labels= ["L2O (q=2)", "BFGS (q=2)", "L2O (q=3)", "BFGS (q=3)"]
elif type == "wide":
    L2O_MAIN = "results_L2O/wide_data.npz"
    L2O_VAL2 = "results_L2O/wide_data2.npz"
    BFGS_MAIN = "results_L2O/BFGS_wide.npz"
    BFGS_VAL2 = "results_L2O/BFGS_wide2.npz"
    labels= ["L2O (q=1)", "BFGS (q=1)", "L2O (q=2)", "BFGS (q=2)"]
...
ax_train.plot(x, m_train, lw=1.0, label=labels[0])
...
if bfgs_train is not None:
    ...
    ax_train.plot(x, m_b_train,  lw=1.0, label= labels[1])
    ax_train.fill_between(x, q1_b_train, q3_b_train, alpha=.2)
...
ax_test.plot(x, m_test, lw=1.0, label=labels[0])
ax_test.fill_between(x, q1_test, q3_test, alpha=.2)

if bfgs_test is not None:
    m_b_test, q1_b_test, q3_b_test = stats(bfgs_test)
    ax_test.plot(x, m_b_test, lw=1.0, label= labels[1])
    ax_test.fill_between(x, q1_b_test, q3_b_test, alpha=.2)
...
ax_val.plot(x, m_val, lw=1.0, label=labels[0])
ax_val.fill_between(x, q1_val, q3_val, alpha=.2)

if bfgs_val is not None:
    m_b_val, q1_b_val, q3_b_val = stats(bfgs_val)
    ax_val.plot(x, m_b_val, lw=1.0, label= labels[1])
    ax_val.fill_between(x, q1_b_val, q3_b_val, alpha=.2)

# Extra quality=3 curves for validation
if val2_l2o is not None:
    m_val2, q1_val2, q3_val2 = stats(val2_l2o)
    ax_val.plot(x, m_val2, lw=1.0, label= labels[2])
    ax_val.fill_between(x, q1_val2, q3_val2, alpha=.2)

if bfgs_val2 is not None:
    m_b_val2, q1_b_val2, q3_b_val2 = stats(bfgs_val2)
    ax_val.plot(x, m_b_val2, lw=1.0, label= labels[3])
    ax_val.fill_between(x, q1_b_val2, q3_b_val2, alpha=.2)
...
plt.savefig("results_L2O/plot_%s.pdf"%type, dpi=300, bbox_inches="tight")
plt.show()
